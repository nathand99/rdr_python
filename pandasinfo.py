import numpy as np 
import pandas as pd
# numpy is adependancy for pandas
...
# 1. manually load hard-coded data into a dataframe
...
# 2. read text file into a dataframe (same result as 1.)
...
print(df.avg_low)							# index with single column (avg_low column values_ - using .

header("7. slicing -- df['avg_low']")
print(df['avg_low'])						# index with single column - use [] - same as above

header("7. slicing -- df[2:4]")				
print(df[2:4])								# rows 2 to 3

header("7. slicing -- df[['avg_low','avg_high']]")		
print(df[['avg_low','avg_high']])			# index with 2 columns

header("7. slicing -- df.loc[:,['avg_low','avg_high']]")
print(df.loc[:,['avg_low','avg_high']])		# multiple columns: df.loc[from_row:to_row,['column1','column2']] - same as above
